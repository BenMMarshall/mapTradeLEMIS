...
plot_species_residence <- function(lemisDataRenamed, lemisISOconversion,
  ...
  plotList_captive <- vector("list", length = length(grpVector))
  names(plotList_captive) <- grpVector
  for(grp in grpVector){
    ...
    lemisGroup_long_wild <- lemisGroup_dist %>%
    ...
    lemisGroup_long_wild <- lemisGroup_long_wild %>%
      ungroup() %>%
    ...
    lemisGroup_plotdata_wild <- lemisGroup_long_wild %>%
    ...
    lemisGroup_plotdata_wild <- lemisGroup_plotdata_wild %>%
    ...
    write.csv(lemisGroup_plotdata_wild, here("tables", paste0("residenceWildSpecies_", grp, ".csv")),
              row.names = FALSE)
    ...
    worldData_nSpp <- worldDataList$worldData %>%
      left_join(lemisGroup_plotdata_wild %>%
    ...
    radiusScaled <- scale(lemisGroup_plotdata_wild$nSpp_total, center = FALSE)* radiusModifier
    # radiusScaled <- radiusScaled
    # radiusScaled * attr(radiusScaled, 'scaled:scale') / 2

    lemisGroup_plotdata_wild$scaledSpp <- scale(lemisGroup_plotdata_wild$nSpp_total, center = FALSE)[,1]
    ...
    completePlot <-
      ...
      geom_curve(data = lemisGroup_plotdata_wild,
                 aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
                     xend = -100,
                     y = centre_y_origin,
                     yend = 40,
      ...
      #            # fill = "black"
      # ) +
      geom_point(data = lemisGroup_plotdata_wild %>%
                   filter(scaledSpp < 1),
                 aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
                     y = centre_y_origin),
                 size = 0.35,
      ...
      geom_scatterpie(data = lemisGroup_plotdata_wild,
                      aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
      ...
      geom_scatterpie_legend(scale(lemisGroup_plotdata_wild$nSpp_total, center = FALSE)* radiusModifierLegend,
                             n = 4,
                             x = -250, y = -65,
    ...
    lemisGroup_long_captive <- lemisGroup_dist %>%
      filter(originCapWild == "Captive") %>%
      group_by(corrected, nativeOrigin, iso2) %>%
      slice_head(n = 1) %>%
      group_by(iso2, nativeOrigin) %>%
      summarise(nSpp = n_distinct(corrected)) %>%
    ...
    lemisGroup_long_captive <- lemisGroup_long_captive %>%
      ungroup() %>%
      tidyr::complete(iso2, nativeOrigin, fill = list(nSpp = 0)) %>%
      group_by(iso2) %>%
      mutate(centre_y_origin = centre_y_origin[!is.na(centre_y_origin)][1],
    ...
    lemisGroup_plotdata_captive <- lemisGroup_long_captive %>%
      mutate(nativeOrigin = case_when(
        nativeOrigin == TRUE ~ "Resident",
        nativeOrigin == FALSE ~ "NonResident",
        is.na(nativeOrigin) ~ "Unknown"
    ...
    lemisGroup_plotdata_captive <- lemisGroup_plotdata_captive %>%
      left_join(lemisGroup_dist %>%
                  group_by(corrected, iso2) %>%
                  slice_head(n = 1) %>%
                  group_by(iso2) %>%
    ...
    write.csv(lemisGroup_plotdata_captive, here("tables", paste0("residenceCaptiveSpecies_", grp, ".csv")),
              row.names = FALSE)
    ...
    lemisGroup_mismatches <- lemisGroup_dist %>%
      ...
      filter(nativeOrigin == "NonResident" & originCapWild == "Captive") %>%
      rename("nSppNonResidentcaptiveMismatch" = nSpp) %>%
      left_join(lemisGroup_dist %>%
                  group_by(corrected, iso2) %>%
                  slice_head(n = 1) %>%
                  group_by(iso2) %>%
    ...
    write.csv(lemisGroup_mismatches, here("tables", paste0("residenceCaptiveMismatchSpecies_", grp, ".csv")),
              row.names = FALSE)

    worldData_nSpp <- worldDataList$worldData %>%
      left_join(lemisGroup_plotdata_captive %>%
                  select(iso2, nSpp_total))
    wrappedData_nSpp <- worldDataList$wrappedData %>%
      left_join(lemisGroup_plotdata_captive %>%
                  select(iso2, nSpp_total))
    ...
    radiusModifier <- 2.5
    radiusModifierLegend <- 2.25
    radiusScaled <- scale(lemisGroup_plotdata_captive$nSpp_total, center = FALSE)* radiusModifier
    # radiusScaled <- radiusScaled
    # radiusScaled * attr(radiusScaled, 'scaled:scale') / 2

    lemisGroup_plotdata_captive$scaledSpp <- scale(lemisGroup_plotdata_captive$nSpp_total, center = FALSE)[,1]

    lemisGroup_plotdata_captive <- lemisGroup_plotdata_captive %>%
      mutate(scaledSpp = scaledSpp* radiusModifier)

    completePlot_captive <-
      world_gg +
      new_scale_fill() +
      # geom_curve(data = routeLineData,
      #            aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
      #                xend = if_else(port_lon > 90, -180-(180-port_lon), port_lon),
      geom_curve(data = lemisGroup_plotdata_captive,
                 aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
                     xend = -100,
                     y = centre_y_origin,
                     yend = 40,
      ...
      #            # fill = "black"
      # ) +
      geom_point(data = lemisGroup_plotdata_captive %>%
                   filter(scaledSpp < 1),
                 aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
                     y = centre_y_origin),
                 size = 0.35,
      ...
      geom_scatterpie(data = lemisGroup_plotdata_captive,
                      aes(x = if_else(centre_x_origin > 90, -180-(180-centre_x_origin), centre_x_origin),
                          y = centre_y_origin, group = iso2,
                          r = scaledSpp),
                      sorted_by_radius = TRUE, linewidth = 0.15,
                      cols = c("Resident", "NonResident", "Unknown"), color = "white", alpha = 0.8) +
      geom_scatterpie_legend(scale(lemisGroup_plotdata_captive$nSpp_total, center = FALSE)* radiusModifierLegend,
                             n = 4,
                             x = -250, y = -65,
                             labeller = function(x){round(x* attr(radiusScaled, 'scaled:scale') / radiusModifier,
                                                          digits = 0)},
    ...
    plotList_captive[[grp]] <- completePlot_captive
  ...
  ggsave(plot = allFacets, filename = here("figures", "distributionWildOriginPlot.png"),
         width = 400, height = 200, units = "mm", dpi = 300)
  ggsave(plot = allFacets, filename = here("figures", "distributionWildOriginPlot.pdf"),
         width = 400, height = 200, units = "mm")

  allFacets_captive <- patchwork::wrap_plots(
    plotList_captive[c("Terrestrial Mammals",
                       "Birds",
                       "Reptiles",
                       "Amphibians",
                       "Arachnids"
  ...
  ggsave(plot = allFacets_captive, filename = here("figures", "distributionCaptiveOriginPlot.png"),
         width = 400, height = 200, units = "mm", dpi = 300)
  ggsave(plot = allFacets_captive, filename = here("figures", "distributionCaptiveOriginPlot.pdf"),
         width = 400, height = 200, units = "mm")
