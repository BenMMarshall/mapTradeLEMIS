#' plot_extinct_purpose
#'
#' @name plot_extinct_purpose
#' @description Mapping plot
#' @return Ggplot2 object and file
#'
#' @export
plot_extinct_purpose <- function(lemisDataRenamed, paletteList){
  ...
  extinctFiles <- list.files(here("data", "iucnData"), pattern = "extinctTradedIUCNdata_",
                             full.names = TRUE)

  extinctTraded <- do.call(rbind, lapply(extinctFiles, read.csv))

  lemisData <- lemisDataRenamed %>%
    filter(rank == "Species" & !corrected == "")

  extinctTradedData <- lemisData %>%
    filter(corrected %in% extinctTraded$corrected |
             lemisName %in% extinctTraded$corrected)


  extinctTradedData <- extinctTradedData %>%
  ...
  write.csv(extinctTradedData, file = here("tables", "extinctTradedData.csv"), row.names = FALSE)
  ...
  ggsave(filename = here("figures", "extinctTraded_purposeDescriptionPlot.png"),
         width = 210, height = 120, units = "mm", dpi = 300)
  ggsave(filename = here("figures", "extinctTraded_purposeDescriptionPlot.pdf"),
         width = 210, height = 120, units = "mm")

  return(extinctTradedData)
